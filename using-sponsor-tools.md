...
Invite the bot and ask it to create or look up a real trip. **Check:** the returned join code opens the same group in the dashboard. For thread context, add facts first and ask for a summary; reading them does not automatically persist member preferences. See [SomeJoy demo prompts](dev-docs/somejoy-demo.md), [the Channel](apps/channel/src/channel.tsx), and [trip tools](apps/channel/src/trip-tools.ts).

**First call, React web:** run `npm run dev:web`, open `http://localhost:3100`, enter a real trip code, then ask: “Where are we going?” **Check:** the answer matches the current trip. [TripControl](apps/web/src/components/trip-control.tsx) registers page context and frontend tools; [GenerativeUI](apps/web/src/components/generative-ui.tsx) registers React components. With Ambiguous configured, ask for a follow-up task proposal, approve it on the page, then refresh and read back the same provider record. This is separate from the map dashboard at port 8000.
...
The [shared MCP connection](packages/agent-core/src/capabilities/workplace.ts) exists, but the current SomeJoy Slack factory disables raw workplace tools. Setting the key alone does not enable them in Slack. The supported approved task flow is in `apps/web`; Ambiguous is not the shared trip database. Tool schemas come from the live workspace; never invent names, arguments, or record URLs. Enforce authorization at the write boundary. A `401` needs valid credentials; a `403` needs appropriate permissions. A new workspace does not fix access to the intended one.
