...
export function TripCard({ headline, summary, facts, nextSteps, tone }: TripCardProps) {
  ...
  return (
    <article className="ck-card" style={{ borderLeftColor: color }}>
      <h3>{headline || "Preparing the trip summary…"}</h3>
      <p>{summary || "Gathering trip details…"}</p>
      {!!facts?.length && (
